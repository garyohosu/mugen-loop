...
## Phase 1.5(マルチエージェント化)の暫定方針(QandA.md #7)

- 構成: Claude Codeをメインエージェントとし、Codex execは読み取り専用のレビュアーとして必要時のみ呼び出す
- ゲート: Codex execは環境変数 `RUN_CODEX=1` が明示的に指定された場合のみ実行する
- 再確認: Codex execの結果は最終判断とせず、Claude Codeが必ず再確認してから報告・提案に反映する
